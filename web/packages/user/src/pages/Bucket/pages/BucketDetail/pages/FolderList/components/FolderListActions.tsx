        ...
        {(onClose) => [
          {
            text: '删除',
            onClick: async () => {
              await deleteFolder({ variables: { data: { bucketName: item.bucketName, path: item.path } } });
              refetch();
              onClose();
            },
          },
      ...
      {(onClose) => [
        {
          ...
          onClick: async () => {
            await deleteObject({
              variables: { data: { bucketName: item.bucketName, path: item.path, filename: item.filename } },
            });
            refetch();
            onClose();
          },
